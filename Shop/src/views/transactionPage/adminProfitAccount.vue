...
export default {
  ...
  created() {
    ...
    axios.post('/getUserTransactions',{username : this.username, accountType : 4})
        .then(response => {
  ...
  methods: {
    filterTransactions7(){
      axios.post('/getUserTransactionsByPeriod',{username : this.username, accountType : 4,periodType :0})
          .then(response => {
            if(response.data.state == 0) {
              this.transactions = response.data.data;
    ...
    filterTransactions30(){
      axios.post('/getUserTransactionsByPeriod',{username : this.username, accountType : 4,periodType :1})
          .then(response => {
            if(response.data.state == 0) {
              this.transactions = response.data.data;
