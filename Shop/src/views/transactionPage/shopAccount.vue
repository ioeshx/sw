...
export default {
  ...
  created() {
    axios.post('/getUserTransactions',{username : this.username, accountType : 2})
        .then(response => {
  ...
  methods: {
    filterTransactions7(){
      axios.post('/getUserTransactionsByPeriod',{username : this.username, accountType : 2,periodType :0})
          .then(response => {
            if(response.data.state == 0) {
              this.transactions = response.data.data;
            }else{
    ...
    filterTransactions30(){
      axios.post('/getUserTransactionsByPeriod',{username : this.username, accountType : 2,periodType :1})
          .then(response => {
            if(response.data.state == 0) {
              this.transactions = response.data.data;
            }else{
